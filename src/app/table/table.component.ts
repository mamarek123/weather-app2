  ...
  public ngOnInit(): void {
    this.weatherEnergyService.forecastData$.subscribe((data) =>{
      if (data) {
          ...
          this.forecastData.generated_energy = parseFloat((data.generated_energy).toFixed(2));
      } else {
    ...
    this.weatherEnergyService.summaryData$.subscribe((data) =>{
      if (data) { 
          ...
          this.summaryData.average_pressure = parseFloat((data.average_pressure).toFixed(2));
      } else {
        console.log("Error fetching data");      }
    })
