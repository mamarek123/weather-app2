...
export class WeatherEnergyService {
  private forecastDataSubject = new BehaviorSubject<ForecastResponse | null>(
    null
  );
  forecastData$ = this.forecastDataSubject.asObservable();

  private summaryDataSubject = new BehaviorSubject<SummaryResponse | null>(
    null
  );
  ...
  fetchSummary(lng: number, lat: number): void {
    ...
    this.http
      .get<SummaryResponse>(this.apiUrl + '/summary', { params })
      .subscribe({
        next: (data) => {
  ...
  fetchForecastAndSummary(lng: number, lat: number): void {
    this.toastr.info(
      'trying to fetch forecast and summary data for location: ' +
        lng +
        ', ' +
        lat
    );
  ...
  fetchForecast(lng: number, lat: number): void {
    ...
    this.http
      .get<ForecastResponse>(this.apiUrl + '/forecast', { params })
      .subscribe({
        next: (data) => {
